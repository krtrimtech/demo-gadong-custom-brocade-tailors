...
(function initHeroScene() {
  ...
  for (let i = 0; i < particleCount; i++) {
    ...
    const color = new THREE.Color(Math.random() > 0.4 ? 0xffffff : 0xa855f7);
    colors[i3] = color.r;
    colors[i3 + 1] = color.g;
    colors[i3 + 2] = color.b;
  ...
  // --- CUSTOM THREE.JS CUSTOM GEOMETRY INJECTION ---
    ...
    // Torus geometries and curved drapes representing luxury fabric fold loops
    function createFabricLoop(color) {
      const geo = new THREE.TorusKnotGeometry(0.8, 0.3, 50, 6, 2, 3);
      const mat = new THREE.MeshStandardMaterial({
        color: color,
        roughness: 0.8,
        metalness: 0.1, // Velvet fabric feel
        flatShading: false
      });
    ...
    for (let i = 0; i < 8; i++) {
      ...
      fabrics.push({
        mesh: fabric,
        rotSpeed: { x: (Math.random() - 0.5) * 0.015, y: (Math.random() - 0.5) * 0.015, z: 0.01 },
        floatSpeed: 0.2 + Math.random() * 0.3,
        floatAmp: 0.4 + Math.random() * 0.8,
        baseY: fabric.position.y
      });
    }
  ...
  const dirLight2 = new THREE.DirectionalLight(0xeab308, 0.8);
  dirLight2.position.set(-10, -5, -5);
  scene.add(dirLight2);

  const pointLight = new THREE.PointLight(0xa855f7, 1.0, 45);
  ...
  function animate() {
    ...
    fabrics.forEach(f => {
      ...
      f.mesh.position.y = f.baseY + Math.sin(elapsed * f.floatSpeed) * f.floatAmp;
    });
